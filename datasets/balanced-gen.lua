...
local function findImages(dir, classList, classToIdx)
   ...
   --- Create a table keyed by classes
   local imageList = {}
   for i, path in ipairs(imagePaths) do
       if imageList[imageClasses[i]] == Nil then
           imageList[imageClasses[i]] = {}
       end
       table.insert(imageList[imageClasses[i]], path)
   end
   return imageList, nImages
end
...
function M.exec(opt, cacheFile)
   ...
   local valImagePath, nValImages = findImages(valDir, classList, classToIdx)
   print(" | Number of validation images: ".. nValImages)

   print(" | finding all training images")
   local trainImagePath, nTrainImages = findImages(trainDir, classList, classToIdx)
   print(" | Number of training images: ".. nTrainImages)
